...
class KeychainHelper {
    ...
    static func load<T: Codable>(key: String, type: T.Type) -> T? {
        ...
        if let data = KeychainHelper.load(key: key) {
            do {
                ...
                print(
                    "KeyChianHelper:Failed to decode \(T.self) key: \(key) , data: \(error.localizedDescription)"
                )
            }
        }
        return nil
    ...
    static func clearSavedData() {
        ...
        if KeychainHelper.load(key: "savedPassword", type: String.self) != nil {
    ...
    static func save(key: String, data: Data) {
        ...
        if status == errSecItemNotFound {
            ...
            if addStatus != errSecSuccess {
                print("⚠️ KeychainHelper: Failed to add item, status = \(addStatus)")
            }
        } else if status != errSecSuccess {
            print("⚠️ KeychainHelper: Failed to update item, status = \(status)")
        }
    }
